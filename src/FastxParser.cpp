...
int parse_read_pairs(
  ...
  while (workQueue.try_dequeue(fn)) {
    ...
    s = &((*local)[numWaiting]);
    while ( (kseq_read(seq)>0) and (kseq_read(seq2)>0)) {

        s->first.seq.s = seq->seq.s;
        s->first.seq.l = seq->seq.l;
        s->first.seq.m = seq->seq.m;
        s->second.seq.s = seq2->seq.s;
        s->second.seq.l = seq2->seq.l;
        s->second.seq.m = seq2->seq.m;
    ...
    // if we had an error in the stream
    // !! Ivan
    if (ks_getc(seq->f) ==-3 or ks_getc(seq2->f) ==-3) {
      --numParsing;
      return -3;
      // !! Ivan
    } else if (ks_getc(seq->f) ==-2 or ks_getc(seq2->f) ==-2) {
      // if we had a quality string of the wrong length
      // tqs == truncated quality string
      --numParsing;
      return -2;
